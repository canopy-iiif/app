import { Callout, Box } from '@canopy-iiif/app/docs-legacy-compat';
...
content will be generated and displayed in the **Contextual References** section. The default layout renders this via the
`<References />` component so individual projects can reposition or restyle the block.
...
To adjust how many tiles render per viewport size—or to express spacing in `rem`—update
`packages/app/ui/src/iiif/sliderOptions.js`. Those defaults cascade everywhere the slider runtime
mounts (homepage facets plus manifest-related sliders), and the bundled helper converts `rem`
values to pixels client-side using `window.getComputedStyle`. If a work page needs a bespoke setup,
pass `sliderOptions` to `<RelatedItems iiifContent={…} sliderOptions={{ breakpoints: … }} />`.
