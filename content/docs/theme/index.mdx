...
The Clover IIIF Viewer (used on work pages) has its own theming system that uses **different CSS variable names**.
Canopy UI components use `--color-accent-*` variables, while Clover uses `--colors-accent` (note the plural "colors"
with no hyphen before "accent").

To customize the viewer's accent colors, add these rules **outside** the `@layer properties` block, targeting the
`.clover-viewer` class directly:
