...
- Optional list of manifest metadata labels (case-sensitive) to facet on.
- Matching is case-insensitive, so `subject`, `SUBJECT`, and `Subject` are treated the same.
...
- Optional list of IIIF Manifest URIs surfaced in the homepage hero and other "featured" placements.
...
- **`tabs.order`** — optional list that reorders the search result tabs. Unknown values are ignored; missing values fall back to the default `work` tab.
