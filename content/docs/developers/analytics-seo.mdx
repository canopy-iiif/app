...
If you only have a single chunk, the index simply references `sitemap-1.xml`, so search engines still find your complete list with the standard entry point.
