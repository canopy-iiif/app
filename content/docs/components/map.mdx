...
| `mapKey`          | `Array<{ id, label }>`                                               | `[]`          | Optional legend configuration used to color-code custom `MapPoint`s by `key`.                                     |
...
| `key`                                 | string             | Optional identifier that links this `MapPoint` to a `mapKey` entry on the parent `<Map>`.                       |
