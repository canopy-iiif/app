...
import {CanopyHeader, CanopyFooter, LanguageToggle} from "@canopy-iiif/app/ui";
...
export function App({children}) {
  return (
    <>
      <CanopyHeader logo={Logo} languageToggle={false} />
      <main>{children}</main>
      <CanopyFooter>
        <LanguageToggle variant="list" />
        <p>Copyright 2025 Bilingual Site, MIT License. A Canopy IIIF project.</p>
      </CanopyFooter>
    </>
  );
}
