...
- 2025-09-27 / chatgpt: Documented Tailwind token flow in `app/styles/tailwind.config.ts`, compiled UI Sass variables during config load, and exposed `stylesheetHref`/`Stylesheet` helpers via `@canopy-iiif/app/head` so `_app.mdx` can reference the generated CSS directly.
